...
lmmModel_estimates.explme <- function(model,
  ...
  trt_names <- as.vector(rbind(trt_names, paste0("SD ", trt_names)))
  
  colnames(dt) <- c(trt_names,"Combination", "SD Combination","SD Random Effects", "SD Residuals")
...
lmmModel_estimates.gompertzlme <- function(model,
  ...
  trt_names_r0 <- as.vector(rbind(trt_names_r0, paste0("SD ", trt_names_r0)))
  trt_names_rho <- as.vector(rbind(trt_names_rho, paste0("SD ", trt_names_rho)))
  trt_names <- c(trt_names_r0, trt_names_rho)
  
  colnames(dt) <- c(trt_names,"SD r0 Random Effects", "SD rho Random Effects","SD Residuals")
